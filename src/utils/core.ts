...
export function getRAmountFromTAmount(chain, amount) {
  ...
  if (isLimitNumber(chain) && amountLength > validDigit) {
    let rAmount = amount.slice(0, validDigit - SIZE_OP.P_NUMBER) + pText + amount.slice(validDigit)
    return {
      state: true,
      rAmount: rAmount,
    }

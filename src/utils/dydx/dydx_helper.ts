...
export class DydxHelper {
   ...
   */
  constructor(chainId: number, web3?: Web3, signingMethod = SigningMethod.TypedData) {
    if (chainId == 11) {
      this.networkId = 1
      this.host = chains_api.dydx.Mainnet
    }
    if (chainId == 511) {
      this.networkId = 3
      this.host = chains_api.dydx.Testnet
    }
  ...
  async getDydxClient(
    ...
    if (ethereumAddress && this.web3) {
      ...
      if (userExists.exists) {
        ...
        const keyPair = await client.onboarding.deriveStarkKey(ethereumAddress, this.signingMethod)
   ...
   * @returns
   */
  async getBalanceUsdc(ethereumAddress: string, ensureUser = true): Promise<ethers.BigNumber> {
    if (!ethereumAddress) {
   ...
   * @returns
   */
  async getAccount(ethereumAddress: string, alwaysNew = false): Promise<AccountResponseObject> {
   ...
   */
  static setApiKeyCredentials(ethereumAddress: string, apiKeyCredentials: ApiKeyCredentials) {

...
import { IMXHelper, ImxTransaction } from './imx_helper'
...
type TransferCallbacks = {
  onConfirmation?: (transaction: ImxTransaction) => any
  onReceived?: (transaction: ImxTransaction) => any
}
...
class IMXListen {
  ...
  constructor(chainId: number, receiver: string | undefined = undefined, isFirstTicker = true) {
...
export function factoryIMXListen(
  ...
  if (factorys[factoryKey]) {
    ...
    return (factorys[factoryKey] = new IMXListen(chainId, receiver, isFirstTicker))
  }
}
